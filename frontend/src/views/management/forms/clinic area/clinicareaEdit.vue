<template>
  <div>
    <div class="jumbotron dashboard">
      ...
      <div v-else class="dashlabel">Adding New Clinic Area</div>
    </div>
    ...
    <div class="editForm">
      ...
      <div class="editFormFooter-right">
        ...
        <button
          ...
          :disabled="validationFormCheck === false"
          v-on:click="addClinicArea"
        >
          Submit New Clinic Area
        </button>
      </div>
    </div>
    ...
    <form class="swal2-form mainForm">
      <div class="editForm-left">
        <FormulateInput
          ...
          :validation-messages="{
            required: 'The Clinic Area Name is required',
          }"
        />
      </div>
      <div class="editForm-left">
...
export default {
  ...
  methods: {
    ...
    addClinicArea() {
      axios
        .post(`${config.api}/api/Clinic_Area/create`, this.form.model)
        .then((response) => {
        ...
        .catch(() => {
          Swal.fire(
            'Error',
            'Something went wrong (creating Clinic Area)',
            'error',
          )
        })
    },
    updateClinicArea() {
      ...
      axios
        .put(
          `${config.api}/api/Clinic_Area/update/` + ClinicID,
          this.form.model,
        )
        .then((response) => {
          this.loadData()
          Swal.fire('Done!', 'The Clinic Area has been updated.', 'success')
          this.goBack()
        })
        .catch(() => {
          Swal.fire(
            'Error',
            'Something went wrong (updating Clinic Area)',
            'error',
          )
        })
    },
    deleteClinicArea() {
      const ClinicID = this.clinicID
      axios
        .delete(`${config.api}/api/Clinic_Area/delete/` + ClinicID)
        .then((response) => {
          Swal.fire('Done!', 'The Clinic Area has been deleted.', 'success')
          this.goBack()
        })
        .catch(() => {
          Swal.fire(
            'Error',
            'Something went wrong (deleting Clinic Area)',
            'error',
          )
        })
    },
    loadData() {
      axios
        .get(`${config.api}/api/Clinic_Area/find/` + this.clinicID)
        .then((response) => {
          ...
          ;(this.form.model.ClinicAreaName = response.data.clinicAreaName),
            ...
            (this.form.model.ClinicAreaLeadPhone =
              response.data.clinicAreaLeadPhone),
            (this.form.model.ClinicAreaLeadEmail =
              response.data.clinicAreaLeadEmail)
          this.form.model.HospitalID = response.data.hospitalID
        })
        .catch(() => {
          Swal.fire(
            'Error',
            'Something went wrong (finding Clinic Area)',
            'error',
          )
        })
    },

<template>
  <div>
    <div class="jumbotron dashboard">
      ...
      <div v-else class="dashlabel">Adding New Contractor Project</div>
    </div>
    ...
    <div class="editForm">
      ...
      <div class="editFormFooter-right">
        ...
        <button
          ...
          v-on:click="deleteContractorProject"
        >
        ...
        <button
          ...
          :disabled="validationFormCheck === false"
          v-on:click="addContractorProject"
        >
          Submit Contractor Project
        </button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    addContractorProject() {
      axios
        .post(`${config.api}/api/Contractor_Project/create`, this.form.model)
        .then((response) => {
        ...
        .catch(() => {
          Swal.fire(
            'Error',
            'Something went wrong (creating Contractor Project)',
            'error',
          )
        })
    },
    updateContractorProject() {
      ...
      axios
        .put(
          `${config.api}/api/Contractor_Project/update/` + ContractorID,
          this.form.model,
        )
        .then((response) => {
          this.loadData()
        ...
        .catch(() => {
          Swal.fire(
            'Error',
            'Something went wrong (updating Contractor Project) or there is repeating data',
            'error',
          )
        })
    },
    deleteContractorProject() {
      ...
      axios
        .delete(
          `${config.api}/api/Contractor_Project/delete/` +
            ContractorID +
            ProjectID,
        )
        .then((response) => {
        ...
        .catch(() => {
          Swal.fire(
            'Error',
            'Something went wrong (deleting Contractor Project)',
            'error',
          )
        })
    },
    ...
    //       Swal.fire(
    //         'Error',
    //         'Something went wrong (finding Contractor Project)',
    //         'error',
    //       )
    //     })
    // },
    loadFields() {
      axios
        ...
        .catch(() => {
          Swal.fire(
            'Error',
            'Something went wrong (loading Contractor)',
            'error',
          )
        })
      axios
  ...
  beforeMount() {
    this.loadFields()
    if (
      this.Contractor.contractorID !== undefined &&

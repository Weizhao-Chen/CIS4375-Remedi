...
router.get('/find', (req, res, next) => {
    ...
    return db.Contractor.findAll({
        include: [
            db.Contractor_Status
        ]
    })
        .then((Contractor) => res.send(Contractor))
        .catch((err) => {
            console.log('There was an error querying Contractor 1', JSON.stringify(err))
            return res.send(err)
        });
})
...
router.get('/find/:contractorid', (req, res, next) => {
    ...
    return db.Contractor.find({
        ...
        .catch((err) => {
            console.log('There was an error querying Contractor 2', JSON.stringify(err))
            return res.send(err)
        });
})

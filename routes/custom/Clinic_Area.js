...
router.get('/find', (req, res, next) => {
    ...
    return db.Clinic_Area.findAll({
        ...
        .catch((err) => {
            console.log('There was an error querying Clinic Area 1', JSON.stringify(err))
            return res.send(err)
        });
})
...
router.get('/find/:clinicid', (req, res, next) => {
    ...
    return db.Clinic_Area.find({
        ...
        .catch((err) => {
            console.log('There was an error querying Clinic Area 2', JSON.stringify(err))
            return res.send(err)
        });
})
router.post('/create', (req, res, next) => {
    ...
    db.Clinic_Area.create({
        ...
        .catch(err => {
        console.log('There was an error creating Clinic Area', JSON.stringify(err))
        return res.send(err)
    })
})
router.put('/update/:clinicid', (req, res, next) => {
    ...
    db.Clinic_Area.update({
        ...
        .catch(err => {
           console.log('There was an error updating Clinic Area', JSON.stringify(err))
           return res.send(err)
   })
})
router.delete('/delete/:clinicid', (req, res, next) => {
    ...
    db.Clinic_Area.destroy({
        ...
        console.log('There was an error deleting Clinic Area', JSON.stringify(err))
        return res.send(err)
    });
})
